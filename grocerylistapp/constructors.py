...
class CompiledIngredientLine:
    def __init__(self, cleaned_line):
        ...
        self.ingredient_id = 'id-' + ingredient_id
...
# card for the ingredient list to show on the user's page
class ChecklistCard:
...
def create_recipe(title):
    ...
    rlist = RecipeList(hex_name=random_hex, hex_color=hex_color, name=title)
...
def create_recipe_from_url(url):
    ...
    rlist = create_recipe(recipe_info.get("title", ""))
    rlist.recipe_url = url

    for num, line in enumerate(recipe_info.get('recipe_lines', '')):
    ...
    if recipe_info.get("error"):
        if recipe_info["error"] == 403:
            flash("The website for your recipe returned an error code of 403. This most likely means that there are" +
                  " anti-scraping measures in place.", "danger")
        else:
            flash("There was a problem with your recipe and the parser could not parse it. Error code: "
                  + str(recipe_info["error"]), "danger")
